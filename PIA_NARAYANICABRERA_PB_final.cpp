...
void alta()
{
    ...
    for (int i = 0; i < alt; i++)
    {
        ...
        while(numeroRepetido)
        {
         ...
         for(int j = 0; j < i; j++)
         {
        	if (Tienda[j].n == Tienda[i].n)
        	{
        		printf("El numero de registro ya existe, por favor ingrese uno diferente %c \n", 01);
        		numeroRepetido = true;
        		break;		
		   	}
		 }
...
{
	int filtro;
	string filtroGen, filtroClas, filtroCon;
        printf("\n1-.Genero \n2-.CLasificacion \n3-.Consola \nPara ver la lista completa escriba 4 \n Seleccione el filtro: ");
        cin >> filtro;
        ...
        switch(filtro)
		{
        	case 1:
        		...
        		getline(cin, filtroGen);
        		for (int i = 0; i < alt; i++)
				{
        			if(Tienda[i].gen == filtroGen && Tienda[i].status != "T-T eliminado T-T")
					{
						printf("\n");
        				printf("%c Articulo %d \n", 03, i + 1);
                        printf("%c Numero: %d \n", 03, Tienda[i].n);
		    ...
		    case 2:
		    	while (getchar() != '\n');
		    	printf("\nQue Clasificacion desea buscar: ");
        		getline(cin, filtroClas);
        		for (int i = 0; i < alt; i++)
				{
        			if(Tienda[i].clas == filtroClas && Tienda[i].status != "T-T eliminado T-T")
					{
						printf("\n");
        				printf("%c Articulo %d \n", 03, i + 1);
                        printf("%c Numero: %d \n", 03, Tienda[i].n);
		    ...
		    case 3:
		    	while (getchar() != '\n');
		    	printf("\nQue Consola desea buscar: ");
        		getline(cin, filtroCon);
        		for (int i = 0; i < alt; i++)
				{
        			if(Tienda[i].con == filtroCon && Tienda[i].status != "T-T eliminado T-T")
					{
						printf("\n");
        				printf("%c Articulo %d \n", 03, i + 1);
                        printf("%c Numero: %d \n", 03, Tienda[i].n);
